...
void ESP32_SendCmd(uint8_t* cmd)
{
    ...
    memset(recvBuff, 0, recvDataLen);  // 清空缓冲区

    HAL_UART_Transmit(&huart2, cmd, len, 1000);


    uint32_t startTime = HAL_GetTick();
    const uint32_t timeout = 80000;  // 超时 5 秒

    do
    {
        ESP32_ReadResp(recvBuff + count, &recvDataLen);
        count = recvDataLen;
        if (HAL_GetTick() - startTime > timeout)  // 超时处理
        {
            printf("ESP32 响应超时！\n");
            printf("ESP32 返回: %.*s\n", recvDataLen, recvBuff);
            return;
        }
    } while (strstr((char*)recvBuff, "OK") == NULL);
    count = 0;
    printf("ESP32 返回: %.*s\n", recvDataLen, recvBuff);  // 打印完整 ESP32 返回值

}

//读取响应
void ESP32_ReadResp(uint8_t recvBuff[], uint16_t* recvDataLen)
{
    HAL_UARTEx_ReceiveToIdle(&huart2, recvBuff, 1000, recvDataLen, 1000);
}
